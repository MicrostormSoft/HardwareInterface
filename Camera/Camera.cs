...
namespace HardwareInterface
{
    ...
    public partial class Camera : ISensor
    {
        private Mat src;
        //private FrameSource cap;
        public VideoCapture cap;
        private Thread camthread;
        public Camera(int cam_id = 0, VideoCaptureAPIs vpis = VideoCaptureAPIs.ANY)
        {
            src = new Mat();
        ...
        public Camera(NvArgusCameraSrc.NvCamSettings settings)
        {
            src = new Mat();
            cap = new VideoCapture(NvArgusCameraSrc.GetDeviceGStream(settings), VideoCaptureAPIs.GSTREAMER)
            {
                ConvertRgb = true
            };
            camthread = new Thread(new ThreadStart(() => { while (true) { try { CapNextFrame(); } catch (Exception ee) { Console.WriteLine(ee.Message + "\n" + ee.StackTrace); } } }));
